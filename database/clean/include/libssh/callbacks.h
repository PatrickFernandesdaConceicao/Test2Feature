 ...
 */
#define ssh_callbacks_execute_list(list, cbtype, c, __VA_ARGS__) do { struct ssh_iterator *i = ssh_list_get_iterator(list); cbtype cb; while (i != NULL){ cb = ssh_iterator_value(cbtype, i); if (ssh_callbacks_exists(cb, c)) cb-> c (__VA_ARGS__, cb->userdata); i = i->next; } } while(0)
 ...
 * @endcode
 */
#define ssh_callbacks_iterate(_cb_list, _cb_type, _cb_name) do { struct ssh_iterator *_cb_i = ssh_list_get_iterator(_cb_list); _cb_type _cb; for (; _cb_i != NULL; _cb_i = _cb_i->next) { _cb = ssh_iterator_value(_cb_type, _cb_i); if (ssh_callbacks_exists(_cb, _cb_name))
...
#define ssh_callbacks_iterate_exec(_cb_name, __VA_ARGS__) _cb->_cb_name(__VA_ARGS__, _cb->userdata)


#define ssh_callbacks_iterate_end() } } while(0)
...
#endif /*_SSH_CALLBACK_H */ /*_SSH_CALLBACK_H */
