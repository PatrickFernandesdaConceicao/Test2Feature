...
#define bignum_safe_free(num) do { if ((num) != NULL) { gcry_mpi_release((num)); (num)=NULL; } } while (0)
...
#define bignum_bn2hex(num, data) gcry_mpi_aprint(GCRYMPI_FMT_HEX, data, NULL, (const gcry_mpi_t)num)
...
#define bignum_dup(orig, dest) do { if (*(dest) == NULL) { *(dest) = gcry_mpi_copy(orig); } else { gcry_mpi_set(*(dest), orig); } } while(0)
...
#endif /* LIBGCRYPT_H_ */ /* LIBGCRYPT_H_ */
