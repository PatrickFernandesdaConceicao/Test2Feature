...
#define bignum_safe_free(num) do { if ((num) != NULL) { ssh_mbedcry_bn_free(num); (num)=NULL; } } while(0)
...
#define bignum_set_word(bn, n) (mbedtls_mpi_lset(bn, n)==0?1:0)
#define bignum_bin2bn(data, datalen, bn) do { *(bn) = bignum_new(); if (*(bn) != NULL) { mbedtls_mpi_read_binary(*(bn), data, datalen); } } while(0)
...
#define bignum_mod_exp(dest, generator, exp, modulo, ctx) (mbedtls_mpi_exp_mod(dest, generator, exp, modulo, NULL)==0?1:0)

#define bignum_add(dest, a, b) mbedtls_mpi_add_mpi(dest, a, b)
#define bignum_sub(dest, a, b) mbedtls_mpi_sub_mpi(dest, a, b)
#define bignum_mod(dest, a, b, ctx) (mbedtls_mpi_mod_mpi(dest, a, b) == 0 ? 1 : 0)
...
#define bignum_bn2bin(num, len, ptr) mbedtls_mpi_write_binary(num, ptr, mbedtls_mpi_size(num))
...
#define bignum_dup(orig, dest) do { if (*(dest) == NULL) { *(dest) = bignum_new(); } if (*(dest) != NULL) { mbedtls_mpi_copy(orig, *(dest)); } } while(0)
...
#endif /* HAVE_LIBMBEDCRYPTO */ /* HAVE_LIBMBEDCRYPTO */
#endif /* LIBMBEDCRYPTO_H_ */ /* LIBMBEDCRYPTO_H_ */
